...
class BusinessCalendar
{
    ...
    const SATURDAY  = 6;
    const SUNDAY    = 7;
    const WEEK_DAY_FORMAT = 'N'; // repeat every week
    const HOLIDAY_FORMAT  = 'Y-m-d'; // repeat every year by using 'm-d' however some holidays are different every year
    ...
    /** @var int[] */
    private $freeWeekDays = array(BusinessCalendar::SATURDAY, BusinessCalendar::SUNDAY);

    public function __construct($freeWeekDays = [], $holidays = [])
    {
    ...
    public function isHoliday(\DateTime $date)
    {
        ...
        foreach ($this->getHolidays() as $holiday) {
            $holidayDate =  \DateTime::createFromFormat('!d/m/Y', $holiday);
            if ($holidayFormatValue == $holidayDate->format(self::HOLIDAY_FORMAT)) {
                return true;
            }
        }
        return false;
