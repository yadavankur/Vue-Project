<template>
<div>   
    <div><cpmactivity-crud-modal></cpmactivity-crud-modal>
            <associated-user-modal
                :activityId="activityId"  :serviceId="serviceId"  :isShowAssociatedUser="isShowAssociatedUser"
                @onCloseAssociatedUserModal="onCloseAssociatedUserModal"
            >
            </associated-user-modal>
            <div class="pull-left">
                <cpm-activity-custom-filter-bar  @onClickNew="onClickNew"></cpm-activity-custom-filter-bar>
            </div>
...
<script>
    ...
    import CpmActivityCustomActions from './Cpm1ActivityCustomActions.vue'
    import CpmActivityCustomFilterBar from './Cpm1ActivityCustomFilterBar.vue'
    import CpmActivityCrudModal from './Cpm1ActivityCrudModal.vue'
    ...
    export default 
    {
        computed: {     url(){   return api.currentCpm1ActivityNodes;    }    },
        props: {      },
        components: 
        ...
        { return {       paginationPath: '',    search: '',       perPage: 5,
                fields: 
                [   //all these files are coming from cpm1repository-----via api-------
                    //also---service_group_activity---is a function used in repo--defined in model cpm1
                    //Actions------defined in --__component:cpm-activity-custom-actions
                    ...
                    {   title: 'Activity Name',   name: 'name',  sortField: 'cpm1s.name',   },
                    ...
                    {   title: 'Actions',    name: '__component:cpm-activity-custom-actions',  titleClass: 'text-center',   dataClass: 'text-center'  }
                ],
                css: {  table: 
                ...
                sortOrder: [ { field: 'name', sortField: 'cpm1s.id', direction: 'asc'} ],
                moreParams: {}, isShowAssociatedUser: false, activityId: '', serviceId: '',
            }
        },
        methods: 
        {   openAssociatedUsersModal() { this.isShowAssociatedUser = true;  },
            ...
            onClickNew() 
            {   console.log('Cpm1ActivityListTable-- onClickNew');
                let payload = {  isShow: true, data: {action: 'Add', data: null, index: ''},  };
                this.service_group = '';
                this.$store.dispatch('setCpm1ActivityShowModal', payload); //----this will to to store--
                //then---it will go to watcher
            },
            ...
            onActions (data) 
            {   console.log('/cpm1/CpmActivityListTable.vue-onActions', data);
                let payload = { isShow: true, data: data, };
                if (data.action === 'Delete')
                {   let me = this;
                    this.$swal({
                        title: 'Are you sure?',
                        text: 'You will not be able to recover this cpm1 service!',
                        type: 'warning',
                    ...
                    {   me.$store.dispatch('deleteCpm1ActivityRequest', data.data)
                            .then((response) => 
                            {   console.log('/cpm1/cpm1activitlisttable.vue-methods-onaction-delete -> refreshCpmActivityTable');
                                me.$events.fire('refreshCpm1ActivityTable');
                            })
                            .catch((error) => {});
                        }, function (dismiss) {    });
                }
                ...
                {
                    console.log('/cpm1/cpm1activitlisttable.vue-methods-onaction-edit*** data.data.service_group_activity=', data.data.service_group_activity);
        ...
        {  'cpm-activity-filter-set' (filterData) 
                ...
                Vue.nextTick( () => {  if (this.$refs.cpmActivityvuetable)   this.$refs.cpmActivityvuetable.refresh()  } )
                },
            'cpm-activity-filter-reset' () 
                {   this.moreParams = {}
                    Vue.nextTick( () => {   if (this.$refs.cpmActivityvuetable) this.$refs.cpmActivityvuetable.refresh()  } )
                },
            'refreshCpm1ActivityTable'() 
                {    console.log('/cpm1/cpm1activitlisttable.vue-events-> refreshCpm1ActivityTable');
                    Vue.nextTick( () => {    if (this.$refs.cpmActivityvuetable)   this.$refs.cpmActivityvuetable.refresh() } );
                }
        },
