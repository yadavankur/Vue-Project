<template>
    <div>
        <div class="test-custom-actions text-center">
...
export default 
 { components: {    },
   props: {  item:   {  type: Object, },  rowData: { type: Object, required: true  },  rowIndex: {   type: String,  },},
   data () {  return {  actAdd: false,  //---not used
                        actEdit: {  type: Boolean, default: true    },
                        actDelete: {  type: Boolean, default: true   },
                      }
            },
   mounted() {     },
   updated() {  console.log('cs/csttype/CsStatusCustomActions.vue- Component updated.');      },
   created() { console.log('cs/csttype/CsStatusCustomActions.vue--- created: ', this.rowData, this.rowIndex);  },
   methods: {  itemAction (action, data, index) //--edit-1-----then to csstatuslistview.vue
                {    console.log('cs/csttype/CsStatusCustomActions.vue---itemAction: ' + action, data, index); 
                     this.$emit('onActions', {action: action, data: data, index: index});// emit event to let parent do things
                },
             },
     watch:   {   }
    }
