...
//    import AppButtonSpinner from '../../AppButtonSpinner.vue';
    ...
    export default
     { computed: {  ...mapGetters({    }), ...mapState({  user: state => state.authUser, }),   },
       ...
       created() 
         { console.log('/booking/orderlist/OrderListSearchSection.vue-OrderListSearchSection vue created!');
           this.$store.dispatch('getOrderStatusOptions')
           .then((response) =>
               { console.log('/booking/orderlist/OrderListSearchSection.vue-created getOrderStatusOptions success=', response);
                 this.setOrderStatusOptions(response.data);
               })
           .catch((error) => {  console.error('/booking/orderlist/OrderListSearchSection.vue-getAssignedLocationOptions error=', error);     });
            this.$store.dispatch('getAssignedLocationOptions')
                .then((response) => {
                    console.log('/booking/orderlist/OrderListSearchSection.vue-created getAssignedLocationOptions success=', response);
        ...
        {  onChangeDateRange(val) 
           {   console.log('/booking/orderlist/OrderListSearchSection.vue-onChangeDateRange val=',val);
                this.formSearchData.dateRange = val;
            },
           onChangeLocation(val) {  console.log('/booking/orderlist/OrderListSearchSection.vue-onChangeLocation val=',val);
            },
            onChangeStatus(val) { console.log('/booking/orderlist/OrderListSearchSection.vue-onStatusLocation val=',val);
            },
            setOrderStatusOptions(statuses) 
            {   console.log('/booking/orderlist/OrderListSearchSection.vue-setOrderStatusOptions statuses=',statuses);
                let options = [];
                for (let status in statuses) { options.push({value: statuses[status].DESCR, label: statuses[status].DESCR});  }
                this.statusOptions = options;
            },
            setAssignedLocationOptions(locations) 
            {   console.log('/booking/orderlist/OrderListSearchSection.vue-setAssignedLocationOptions locations=',locations);
                let options = [];
                for (let location in locations) {  options.push({value: locations[location].id, label: locations[location].name});    }
                this.locationOptions = options;
            },
            doFilter () 
            {   console.log('/booking/orderlist/OrderListSearchSection.vue-OrderListSearchSection doFilter');
                   //     this.isSubmitted = true;
                this.$events.fire('order-list-filter', this.formSearchData)
            },
            resetFilter () 
            {   console.log('/booking/orderlist/OrderListSearchSection.vue-OrderListSearchSection resetFilter');
                this.resetFormData();
                // after reset, whether passing data or not  // actually is the same
                this.$events.fire('/booking/orderlist/OrderListSearchSection.vue-order-list-reset', this.formSearchData)
            },
            resetFormData() 
              {   this.formSearchData =  
