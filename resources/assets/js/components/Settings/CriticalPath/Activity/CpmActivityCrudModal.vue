...
<script>
    ...
    export default 
    {   computed: {  ...mapState({      showModal: state => state.cpmActivity.showModal,
                                        cpmActivityData: state=> state.cpmActivity.cpmActivityData,
                                }),
                 },
        props: {      },
        data () { return {   title: '',
                            formData: {   name: '',  duration: '',  isFull: true,
                                          tick_option: false,  sql_statement: '',
                                          action_sql: '',   comment: '',
                                          id: '',   service_id: '', service: {name:'', id:''},
                                          service_group: '',
                                          state: {name:'', id:''},
                                          location: {name:'', id:''},
                                      },
        ...
        created() {   console.log('CustomModal Component created.');
                              this.getCascadeLocationServices();
                  },
        components: {  'custom-modal': modal,    'bs-input': input,  },
        mounted() {  console.log('CustomModal Component mounted. cpmActivityData=', this.cpmActivityData)  },
        beforeDestroy() 
        {   console.log('CpmActivityCrudModal beforeDestroy!');
            let payload = { isShow: false,  data: { data: null},   };
            this.$store.dispatch('setCpmActivityShowModal', payload);
        },
        methods: 
        {   onChangeServiceGroup(value) 
            {   console.log('onChangeServiceGroup value=', value);
            ...
            {   this.$store.dispatch('getCpmServiceGroupOptions', payload)
                    .then((response) => 
                    {   console.log('cpmActivityCrudModal getCpmServiceGroupOptions success=', response);
                        this.setServiceGroupOptions(response.data.serviceGroups);
                        if (service_group)
                            this.service_group = service_group;
                    })
                    .catch((error) => {  console.error('cpmActivityCrudModal getCpmServiceGroupOptions error=', error);
                                    });
            },
            setServiceGroupOptions(groups) 
            {   console.log('setServiceGroupOptions groups=',groups);
                let options = [];
                for (let group in groups) 
                {  options.push({value: parseInt(groups[group].value), label: groups[group].label});
                }
            ...
            OnChanged(selVal) {  console.log('OnChanged selVal=', selVal);  },
            OnSave() {   console.log('OnSave');
                ...
                if (this.cpmActivityData.action === 'Add')
                {
                    this.$store.dispatch('addCpmActivityRequest', this.formData)
                        .then((response) => 
                        {   this.$store.dispatch('setCpmActivityShowModal', payload);
                            console.log('addCpmActivityRequest fire events -> refreshCpmActivityTable');
                ...
                else if (this.cpmActivityData.action === 'Edit')
                {
                    ...
                    this.$store.dispatch('updateCpmActivityRequest', this.formData)
                        .then((response) => {
                            this.$store.dispatch('setCpmActivityShowModal', payload);
                            console.log('updateCpmActivityRequest fire events -> refreshCpmActivityTable');
                            this.$events.fire('refreshCpmActivityTable');
                        })
                        .catch((error) => {});
                }
        ...
        watch: {
            cpmActivityData(newVal, oldVal) {
                console.log('+++++++++++++ cpmActivityData changed newVal=', newVal);
                console.log('+++++++++++++ cpmActivityData changed oldVal=', oldVal);
                console.log('+++++++++++++ cpmActivityData changed =', this.cpmActivityData);
                if (this.cpmActivityData && this.cpmActivityData.action === 'Add')
                {
                    this.resetFormData();
                    this.title = 'Adding a new activity';
                }
                else if (this.cpmActivityData && this.cpmActivityData.action === 'Edit')
                {
                    this.resetFormData();
                    this.title = 'Editing the activity';
                    ...
                    this.formData.service = {
                        id : this.cpmActivityData.data.service.id,
                        name: this.cpmActivityData.data.service.name
                    };
                    this.formData.location = {
                        id : this.cpmActivityData.data.service.location.id,
